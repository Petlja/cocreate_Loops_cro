...
XOR *(isključivo ILI)* je logička operacija koja daje istinito (1) samo kada
...
...i na kraju provesti šifriranje — XOR-irajte svaki znak s ključem, ponavljajući
ključ onoliko puta koliko je potrebno:
...
vidio bi samo nečitljive binarne podatke jer znakovi nisu ispisivi.
...
vraća originalne podatke.

U stvarnim primjenama, ponovna upotreba istog ključa za više poruka čini XOR
šifriranje ranjivim na frekventnu analizu i napade poznatog otvorenog teksta.
XOR sam po sebi ne pruža jaku sigurnost osim ako se ključem pravilno upravlja
i nije kraći od poruke — kao kod jednokratne bilježnice (one-time pad). Ipak,
u obrazovne svrhe i za osnovna demonstriranja kriptografskih načela, XOR je
jednostavan i idealan.

## Prvi zadatak

Napravite konzolnu aplikaciju u bilo kojem programskom jeziku koja će šifrirati
i dešifrirati poruke koristeći XOR operaciju. Koristite razvojno okruženje koje
koristite na satovima programiranja.

Dopuštena abeceda za poruke (i za otvoreni tekst i za ključ) sadrži samo mala
...
U prvom retku korisničkog unosa nalazit će se poruka `m` duljine do sto ASCII
znakova za otvoreni tekst ili 800 bita za šifrirani tekst, u drugom retku ključ
`k` duljine do pet znakova, a u trećem retku cijeli broj `s` koji predstavlja
operaciju. Ako je $s=1$, tada je `m` otvoreni tekst i treba biti šifriran, a
ako je $s=2$, tada je `m` šifrirani tekst u binarnom obliku i treba biti dešifriran.
...
ASCII vrijednošću odgovarajućeg znaka ključa i pretvorite rezultat u znak.
...
- opcionalno sadrži privatnu pomoćnu metodu za ponavljanje ključa duž duljine poruke.
...
2. argument `k` za ključ, i
3. argument `s` za operaciju (`1` za šifriranje, `2` za dešifriranje).

### Šifrirajte i dešifrirajte datoteke

Iskoristite dosadašnje znanje za izradu programa koji može:
